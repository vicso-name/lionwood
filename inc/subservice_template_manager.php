<?php
/**
 * Sub-Service CPT — Default Block Template Manager
 *
 * Identical mechanics to service_template_manager.php but targets
 * the `sub_service` CPT with its own block set.
 */
...
add_action('init', function () {
    register_post_type('subservice_template', [
        'labels' => [
            'name'          => __('Sub-Service Templates', 'lionwood'),
            'singular_name' => __('Sub-Service Template', 'lionwood'),
            'edit_item'     => __('Edit Sub-Service Block Template', 'lionwood'),
        ],
        'public'        => false,
        'show_ui'       => true,
        'show_in_menu'  => false,
...
/* ─────────────────────────────────────────────
   2. Settings page under Sub-Services menu
   ───────────────────────────────────────────── */

add_action('admin_menu', function () {
    add_submenu_page(
        'edit.php?post_type=sub_service',
        __('Block Template Settings', 'lionwood'),
        __('Block Template', 'lionwood'),
        'manage_options',
        'subservice-block-template',
        'lionwood_render_subservice_template_settings_page'
    );
});


function lionwood_render_subservice_template_settings_page(): void {
    $template_post_id = lionwood_get_or_create_subservice_template_post();
    $edit_link        = get_edit_post_link($template_post_id, 'raw');

    if (
        isset($_POST['lionwood_subservice_template_nonce']) &&
        wp_verify_nonce($_POST['lionwood_subservice_template_nonce'], 'lionwood_subservice_template_save')
    ) {
        if (isset($_POST['action_reset'])) {
            wp_update_post([
                'ID'           => $template_post_id,
                'post_content' => lionwood_get_subservice_default_template_content(),
            ]);
            ...
            $updated = lionwood_backfill_empty_subservice_posts();
            echo '<div class="notice notice-success"><p>' . sprintf(
                esc_html__('Done. %d sub-service post(s) updated with the block template.', 'lionwood'),
                $updated
            ) . '</p></div>';
        } elseif (isset($_POST['action_fix_edit_mode'])) {
            $updated = lionwood_fix_edit_mode_on_subservice_posts();
            echo '<div class="notice notice-success"><p>' . sprintf(
                esc_html__('Done. Edit mode fixed on %d sub-service post(s).', 'lionwood'),
                $updated
            ) . '</p></div>';
        }
    }

    $empty_count    = lionwood_count_empty_subservice_posts();
    $fix_mode_count = lionwood_count_subservice_posts_needing_edit_mode();
    ?>

    <div class="wrap">
        <h1><?php esc_html_e('Sub-Services — Block Template Settings', 'lionwood'); ?></h1>

        <div class="card" style="max-width: 720px; margin-top: 20px;">
            <h2><?php esc_html_e('Default Block Template', 'lionwood'); ?></h2>
            <p class="description">
                <?php esc_html_e('Define which Gutenberg blocks are pre-loaded when creating a new Sub-Service post. Edit the template in the block editor.', 'lionwood'); ?>
            </p>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <p class="description" style="margin-top: 8px;">
                            <?php esc_html_e('Add, remove, or reorder blocks. Changes apply to all NEW sub-service posts only.', 'lionwood'); ?>
                        </p>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <form method="post">
                            <?php wp_nonce_field('lionwood_subservice_template_save', 'lionwood_subservice_template_nonce'); ?>
        ...
        <div class="card" style="max-width: 720px; margin-top: 20px;">
            ...
            <p>
                <?php printf(
                    esc_html__('%d sub-service post(s) have no content yet and will receive the block template.', 'lionwood'),
                    $empty_count
                ); ?>
            ...
            <form method="post">
                ...
                <button type="submit" name="action_backfill" value="1" class="button button-primary"
                        onclick="return confirm('<?php esc_attr_e('Apply template to all empty sub-service posts? This cannot be undone.', 'lionwood'); ?>')">
        ...
        <?php else : ?>
        <div class="card" style="max-width: 720px; margin-top: 20px;">
            <p style="margin: 0;">✅ <?php esc_html_e('All existing sub-service posts already have content. Nothing to migrate.', 'lionwood'); ?></p>
        </div>
        <?php endif; ?>
        ...
        <div class="card" style="max-width: 720px; margin-top: 20px;">
            ...
            <p>
                <?php printf(
                    esc_html__('%d sub-service post(s) have blocks displayed in Preview mode instead of Edit mode.', 'lionwood'),
                    $fix_mode_count
                ); ?>
            </p>
            <form method="post">
        ...
        <div class="card" style="max-width: 720px; margin-top: 20px; background: #fff8e1; border-left: 4px solid #f0b429;">
            ...
            <ul style="list-style: disc; padding-left: 20px;">
                ...
                <li><?php esc_html_e('All NEW sub-service posts will start with these blocks pre-loaded.', 'lionwood'); ?></li>
...
function lionwood_get_or_create_subservice_template_post(): int {
    $post_id = (int) get_option('lionwood_subservice_template_post_id', 0);
    ...
    $post_id = wp_insert_post([
        'post_title'   => __('Sub-Service — Default Block Template', 'lionwood'),
        'post_type'    => 'subservice_template',
        'post_status'  => 'publish',
        'post_content' => lionwood_get_subservice_default_template_content(),
    ]);

    update_option('lionwood_subservice_template_post_id', $post_id);
...
/* ─────────────────────────────────────────────
   4. Hardcoded default — sub-service block order
   ───────────────────────────────────────────── */

function lionwood_get_subservice_default_template_content(): string {
    return <<<'BLOCKS'
<!-- wp:acf/single-service-hero {"mode":"edit"} /-->
<!-- wp:acf/single-service-definition {"mode":"edit"} /-->
<!-- wp:acf/single-deliver-solutions {"mode":"edit"} /-->
...
function lionwood_subservice_blocks_to_template_array(string $content): array {
...
/* ─────────────────────────────────────────────
   6. Inject template into Sub-Service CPT args
   ───────────────────────────────────────────── */
...
add_filter('register_post_type_args', function (array $args, string $post_type): array {
    ...
    $content = lionwood_get_subservice_default_template_content();
    $tid     = (int) get_option('lionwood_subservice_template_post_id', 0);
    ...
    $args['template']      = lionwood_subservice_blocks_to_template_array($content);
...
add_filter('allowed_block_types_all', function ($allowed, $ctx) {
    if (isset($ctx->post) && $ctx->post->post_type === 'subservice_template') {
        return true;
    }
    return $allowed;
}, 10, 2);
...
add_filter('wp_insert_post_data', function (array $data): array {
    if ($data['post_type'] === 'subservice_template' && !empty($data['post_content'])) {
...
add_filter('user_has_cap', function (array $allcaps, array $caps, array $args): array {
    if (
        ...
        $post_id = $args[2] ?? 0;
        if ($post_id && get_post_type($post_id) === 'subservice_template') {
...
function lionwood_get_empty_subservice_post_ids(): array {
    $posts = get_posts([
...
function lionwood_count_empty_subservice_posts(): int {
    return count(lionwood_get_empty_subservice_post_ids());
}

function lionwood_backfill_empty_subservice_posts(): int {
    $content = lionwood_get_subservice_default_template_content();
    $tid     = (int) get_option('lionwood_subservice_template_post_id', 0);
    ...
    $updated = 0;
    foreach (lionwood_get_empty_subservice_post_ids() as $id) {
        wp_update_post([
            'ID'           => $id,
...
function lionwood_count_subservice_posts_needing_edit_mode(): int {
    $posts = get_posts([
        'post_type'      => 'sub_service',
        'post_status'    => ['publish', 'draft', 'pending', 'future'],
        'posts_per_page' => -1,
...
function lionwood_fix_edit_mode_on_subservice_posts(): int {
    $posts = get_posts([
        'post_type'      => 'sub_service',
        'post_status'    => ['publish', 'draft', 'pending', 'future'],
        'posts_per_page' => -1,
