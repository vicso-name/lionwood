 ...
 * for new Case Study posts via a settings page.
 *
...
add_action('init', function () {
    register_post_type('case_template', [
        'labels' => [
            'name'          => __('Case Templates', 'lionwood'),
            'singular_name' => __('Case Template', 'lionwood'),
            'edit_item'     => __('Edit Case Block Template', 'lionwood'),
        ],
        'public'        => false,
        'show_ui'       => true,
        'show_in_menu'  => false,
...
/* ─────────────────────────────────────────────
   2. Settings page under Cases menu
   ───────────────────────────────────────────── */
...
add_action('admin_menu', function () {
    add_submenu_page(
        ...
        'manage_options',
        'case-block-template',
        'lionwood_render_case_template_settings_page'
    );
});


function lionwood_render_case_template_settings_page(): void {
    $template_post_id = lionwood_get_or_create_case_template_post();
    $edit_link        = get_edit_post_link($template_post_id, 'raw');

    if (
        isset($_POST['lionwood_case_template_nonce']) &&
        wp_verify_nonce($_POST['lionwood_case_template_nonce'], 'lionwood_case_template_save')
    ) {
        if (isset($_POST['action_reset'])) {
            wp_update_post([
                'ID'           => $template_post_id,
                'post_content' => lionwood_get_case_default_template_content(),
            ]);
            ...
            $updated = lionwood_backfill_empty_case_posts();
            echo '<div class="notice notice-success"><p>' . sprintf(
                esc_html__('Done. %d case post(s) updated with the block template.', 'lionwood'),
                $updated
            ) . '</p></div>';
        } elseif (isset($_POST['action_fix_edit_mode'])) {
            $updated = lionwood_fix_edit_mode_on_case_posts();
            echo '<div class="notice notice-success"><p>' . sprintf(
                esc_html__('Done. Edit mode fixed on %d case post(s).', 'lionwood'),
                $updated
            ) . '</p></div>';
        }
    }

    $empty_count    = lionwood_count_empty_case_posts();
    $fix_mode_count = lionwood_count_case_posts_needing_edit_mode();
    ?>

    <div class="wrap">
        <h1><?php esc_html_e('Cases — Block Template Settings', 'lionwood'); ?></h1>

        <div class="card" style="max-width: 720px; margin-top: 20px;">
            <h2><?php esc_html_e('Default Block Template', 'lionwood'); ?></h2>
            <p class="description">
                <?php esc_html_e('Define which Gutenberg blocks are pre-loaded when creating a new Case post. Edit the template in the block editor.', 'lionwood'); ?>
            </p>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <p class="description" style="margin-top: 8px;">
                            <?php esc_html_e('Add, remove, or reorder blocks. Changes apply to all NEW case posts only.', 'lionwood'); ?>
                        </p>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <form method="post">
                            <?php wp_nonce_field('lionwood_case_template_save', 'lionwood_case_template_nonce'); ?>
        ...
        <div class="card" style="max-width: 720px; margin-top: 20px;">
            ...
            <p>
                <?php printf(
                    esc_html__('%d case post(s) have no content yet and will receive the block template.', 'lionwood'),
                    $empty_count
                ); ?>
            ...
            <form method="post">
                ...
                <button type="submit" name="action_backfill" value="1" class="button button-primary"
                        onclick="return confirm('<?php esc_attr_e('Apply template to all empty case posts? This cannot be undone.', 'lionwood'); ?>')">
        ...
        <?php else : ?>
        <div class="card" style="max-width: 720px; margin-top: 20px;">
            <p style="margin: 0;">✅ <?php esc_html_e('All existing case posts already have content. Nothing to migrate.', 'lionwood'); ?></p>
        </div>
        <?php endif; ?>
        ...
        <div class="card" style="max-width: 720px; margin-top: 20px;">
            ...
            <p>
                <?php printf(
                    esc_html__('%d case post(s) have blocks displayed in Preview mode instead of Edit mode.', 'lionwood'),
                    $fix_mode_count
                ); ?>
            </p>
            <form method="post">
        ...
        <div class="card" style="max-width: 720px; margin-top: 20px; background: #fff8e1; border-left: 4px solid #f0b429;">
            ...
            <ul style="list-style: disc; padding-left: 20px;">
                ...
                <li><?php esc_html_e('All NEW case posts will start with these blocks pre-loaded.', 'lionwood'); ?></li>
...
function lionwood_get_or_create_case_template_post(): int {
    $post_id = (int) get_option('lionwood_case_template_post_id', 0);
    ...
    $post_id = wp_insert_post([
        'post_title'   => __('Case — Default Block Template', 'lionwood'),
        'post_type'    => 'case_template',
        'post_status'  => 'publish',
        'post_content' => lionwood_get_case_default_template_content(),
    ]);

    update_option('lionwood_case_template_post_id', $post_id);
...
function lionwood_get_case_default_template_content(): string {
    return <<<'BLOCKS'
<!-- wp:acf/case-hero-section {"mode":"edit"} /-->
<!-- wp:acf/case-client-overview {"mode":"edit"} /-->
<!-- wp:acf/case-results {"mode":"edit"} /-->
<!-- wp:acf/two-column-images {"mode":"edit"} /-->
<!-- wp:acf/business-challenge {"mode":"edit"} /-->
<!-- wp:acf/two-column-images {"mode":"edit"} /-->
<!-- wp:acf/case-solution {"mode":"edit"} /-->
<!-- wp:acf/single-real-solutions {"mode":"edit"} /-->
<!-- wp:acf/banner-section {"mode":"edit"} /-->
<!-- wp:acf/case-core-capabilities {"mode":"edit"} /-->
<!-- wp:acf/case-client-story {"mode":"edit"} /-->
<!-- wp:acf/case-tech-stack {"mode":"edit"} /-->
<!-- wp:acf/banner-section {"mode":"edit"} /-->
<!-- wp:acf/process-timeline {"mode":"edit"} /-->
<!-- wp:acf/case-testimonial {"mode":"edit"} /-->
...
function lionwood_case_blocks_to_template_array(string $content): array {
...
add_filter('register_post_type_args', function (array $args, string $post_type): array {
    ...
    $content = lionwood_get_case_default_template_content();
    $tid     = (int) get_option('lionwood_case_template_post_id', 0);
    ...
    $args['template']      = lionwood_case_blocks_to_template_array($content);
...
function lionwood_get_empty_case_post_ids(): array {
    $posts = get_posts([
...
function lionwood_count_empty_case_posts(): int {
    return count(lionwood_get_empty_case_post_ids());
}

function lionwood_count_case_posts_needing_edit_mode(): int {
    $posts = get_posts([
        'post_type'      => 'case_study',
        'post_status'    => ['publish', 'draft', 'pending', 'future'],
        'posts_per_page' => -1,
...
function lionwood_fix_edit_mode_on_case_posts(): int {
    $posts = get_posts([
        'post_type'      => 'case_study',
        'post_status'    => ['publish', 'draft', 'pending', 'future'],
        'posts_per_page' => -1,
...
function lionwood_backfill_empty_case_posts(): int {
    $content = lionwood_get_case_default_template_content();
    $tid     = (int) get_option('lionwood_case_template_post_id', 0);
    ...
    $updated = 0;
    foreach (lionwood_get_empty_case_post_ids() as $id) {
        wp_update_post([
            'ID'           => $id,
