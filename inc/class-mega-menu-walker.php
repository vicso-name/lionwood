...
class Lionwood_Mega_Menu_Walker extends Walker_Nav_Menu {
    ...
    public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
        ...
        if ( $depth === 0 ) {
            ...
            if ( $has_children ) {
                // Clicking the label navigates like any other top-level item (client
                // requirement — Services/Industries etc. must go to their own page,
                // not just toggle the dropdown). Opening the dropdown itself is a
                // separate control: hover already reveals it on pointer devices
                // (.hdr__menu-item--has-drop:hover in _header.scss, no JS involved),
                // and this button is what lets keyboard users open it too without
                // immediately navigating away — a plain <a> can't do both at once.
                $output .= '<a class="hdr__menu-link" href="' . $url . '"' . $attr_target . $attr_rel . '>';
                $output .= $title;
                $output .= '</a>';
                $output .= '<button type="button" class="hdr__menu-trigger" aria-expanded="false" aria-haspopup="true" aria-label="' . esc_attr( sprintf( __( 'Toggle %s submenu', 'lionwood' ), $item->title ) ) . '" data-hdr-trigger>';
