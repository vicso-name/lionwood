...
// ── UA fallback for untranslated case-study pages ─────────────────────────────
// /uk/case-study/... currently either 404s (no uk term/post at that slug) or —
// worse — gets silently resolved to the EN post/term and then 301'd there by
// WP core's own redirect_canonical() before the page ever renders in Ukrainian.
// Runs at priority 0 so it fires BEFORE redirect_canonical (priority 10) gets a
// chance to hijack the request toward the English URL.
if ( ! function_exists( 'theme_cases_uk_fallback' ) ) {
	function theme_cases_uk_fallback(): void {
		if ( ! function_exists( 'pll_current_language' ) || pll_current_language() !== 'uk' ) return;

		$path = parse_url( $_SERVER['REQUEST_URI'], PHP_URL_PATH ) ?: '';
		if ( strpos( $path, '/uk/case-study/' ) !== 0 ) return;

		$has_uk_equivalent = true;

		if ( is_404() ) {
			$has_uk_equivalent = false;
		} elseif ( is_singular( 'case_study' ) && function_exists( 'pll_get_post_language' ) ) {
			$has_uk_equivalent = pll_get_post_language( get_the_ID() ) === 'uk';
		} elseif ( is_tax( [ 'case_study_category', 'case_study_service' ] ) && function_exists( 'pll_get_term_language' ) ) {
			$term = get_queried_object();
			$has_uk_equivalent = $term && pll_get_term_language( $term->term_id ) === 'uk';
		}
...
add_action( 'template_redirect', 'theme_cases_uk_fallback', 0 );
