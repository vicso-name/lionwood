...
/**
 * The hooks above only cover WP core's own admin_print_scripts/wp_head
 * emoji pipeline. Some admin UI (Polylang/ACF language tabs, flag emoji in
 * tab labels) still renders emoji as <img class="emoji">, and neither of
 * those two hooks catches it — a JS-side DOM-strip approach was tried
 * instead (see git history) but caused an infinite re-render loop with a
 * reactive tab widget and hung wp-admin, so that's off the table.
 *
 * Two safe, hook-based causes instead:
 *
 * 1. TinyMCE bundles its own "wpemoji" plugin, which does the same kind of
 *    emoji-detection independently of admin_print_scripts — untouched by
 *    the remove_action() calls above. Standard fix (same one Twenty
 *    Seventeen's disable-emojis snippet uses): drop it from the plugin list.
 *
 * 2. Whatever CDN URL any surviving wp_staticize_emoji() call resolves to
 *    (via the emoji_svg_url filter) needs to actually work. The jsdelivr
 *    twemoji URL already in use elsewhere on this install
 *    (cdn.jsdelivr.net/gh/jdecked/twemoji@17.0.1/assets/<codepoint>.svg) 404s
 *    every single time — confirmed with curl — because the real package
 *    layout is assets/svg/<codepoint>.svg, missing "/svg/" has nothing to
 *    do with network access. Priority PHP_INT_MAX so this wins over
 *    whatever else sets this filter.
 */
add_filter('tiny_mce_plugins', function ($plugins) {
    return is_array($plugins) ? array_diff($plugins, ['wpemoji']) : $plugins;
});
add_filter('emoji_svg_url', function () {
    return 'https://cdn.jsdelivr.net/gh/jdecked/twemoji@17.0.1/assets/svg/';
}, PHP_INT_MAX);
...
function lionwood_opengraph_namespace( $output ) {
    // Match any existing prefix="..." (e.g. Rank Math adds its own "og: https://ogp.me/ns#"
    // variant) — the old check only looked for the exact "http://" string, missed Rank
    // Math's "https://" one, and produced two duplicate prefix="" attributes on <html>.
...
// WordPress renders <html lang="en-US"> for the English locale (en_US); this
// forces the plain "en" tag SEO asked for without touching Polylang's locale
// setting. Ukrainian already outputs a bare "uk" correctly, untouched here.
add_filter('language_attributes', function ( $output ) {
    return str_replace('lang="en-US"', 'lang="en"', $output);
}, 20);
...
// ── Canonical URL ────────────────────────────────────────────────────────────
// Same situation as the schema block below: Rank Math is installed but isn't
// emitting <link rel="canonical"> on this install (verified via curl — none of
// home/singular/taxonomy pages had one). Self-referencing, including the exact
// requested path (so paginated pages like /page/2/ canonicalize to themselves,
// not to page 1 — current SEO guidance, not the old page-1-only advice).
function lionwood_canonical_link() {
    if ( is_admin() || is_404() ) return;

    global $wp;
    $url = home_url( trailingslashit( $wp->request ) );
...
add_action('wp_head', 'lionwood_canonical_link', 5);
