 ...
 *
 * Latest 3 career posts with same card as career-grid.
 * Mobile: Swiper carousel. Desktop: 3-col grid.
 */
...
$title_top    = get_field( 'title_top' )    ?: 'Latest';
$title_bottom = get_field( 'title_bottom' ) ?: 'Career Possibilities';
...
if ( $source === 'selected' && ! empty( $selected ) ) {
    $posts = array_slice( $selected, 0, 3 );
} else {
    $posts = get_posts( [
        'post_type'      => 'career',
        'post_status'    => 'publish',
...
<section
    class="cp-section"
    style="
        --cp-pt: <?php echo $pt; ?>px;
        --cp-pb: <?php echo $pb; ?>px;
        --cp-pt-mob: <?php echo $pt_mob; ?>px;
        --cp-pb-mob: <?php echo $pb_mob; ?>px;
    "
>
    <div class="cp-section__container">
        ...
        <div class="cp-heading">
            <span class="cp-heading__top"><?php echo esc_html( $title_top ); ?></span>
            <span class="cp-heading__bottom"><?php echo esc_html( $title_bottom ); ?></span>
        </div>
    ...
    <?php if ( ! empty( $posts ) ) : ?>
        ...
        <?php /* ── Desktop: plain grid ── */ ?>
        <div class="cp-section__container">
            <div class="cp-grid cp-grid--desktop">
                ...
                ?>
                    <article class="cg-card" data-cg-card>
                        <div class="cg-card__head">
                            <h3 class="cg-card__title"><?php echo $title; ?></h3>
                            <?php if ( $is_hot ) : ?>
                                <span class="cg-card__hot">HOT <?php echo $fire_svg; ?></span>
                            <?php endif; ?>
                        </div>
        ...
        <?php /* ── Mobile: Swiper carousel ── */ ?>
        <div class="cp-slider swiper cp-grid--mobile" data-cp-slider>
            <div class="swiper-wrapper">
                <?php foreach ( $posts as $post ) :
                    ...
                    $short_out  = $short_desc ? wp_kses_post( $short_desc ) : '';
                ?>
                    <div class="swiper-slide cp-slide">
                        <article class="cg-card" data-cg-card>
                            <div class="cg-card__head">
                                <h3 class="cg-card__title"><?php echo $title; ?></h3>
                                <?php if ( $is_hot ) : ?>
