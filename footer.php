            ...
            <?php if ($footer_support || !empty($footer_support_flag['url'])) : ?>
                <div class="footer-section__support">
                    ...
                    <?php if ($footer_support) :
                        // support_text was switched from a Text field to a Link field;
                        // changing an ACF field's type doesn't convert data already saved
                        // under the old type, so get_field() keeps returning the old plain
                        // string here until someone re-saves it through the new Link UI.
                        // Guard against that the same way cta_section.php guards card_bg.
                        if (is_array($footer_support)) {
                            $footer_support_url    = $footer_support['url'] ?? '';
                            $footer_support_title  = $footer_support['title'] ?: '';
                            $footer_support_target = $footer_support['target'] ?: '_self';
                        } else {
                            $footer_support_url    = '';
                            $footer_support_title  = (string) $footer_support;
                            $footer_support_target = '_self';
                        }
                    ?>
                        <?php if ($footer_support_url) : ?>
                            <a
                                class="footer-section__support-link"
                                href="<?= esc_url($footer_support_url); ?>"
                                target="<?= esc_attr($footer_support_target); ?>"
                                <?= $footer_support_target === '_blank' ? 'rel="noopener noreferrer"' : ''; ?>
                            ><?= esc_html($footer_support_title); ?></a>
                        <?php else : ?>
                            <span><?= esc_html($footer_support_title); ?></span>
                        <?php endif; ?>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
