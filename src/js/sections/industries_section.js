 ...
 *
 * The sticky header (#header, .hdr) is hidden for the duration of the pin —
 * client requirement: the section must occupy the full viewport, and the
 * 72px sticky header would otherwise sit on top of it the whole time. It
 * slides back in once the pin ends (all industries scrolled through) and
 * the page continues past the section, and symmetrically on the way back up.
 *
 * Mobile (<1024px): unchanged — thumbnail click, arrow keys, and a 4s
 * auto-advance timer that pauses on focus. No scroll-jacking, header stays
 * put (no pin exists to protect against).
 *
...
(function () {
  ...
  var stickyBtnEl = document.querySelector('.js-sticky-meeting');
  ...
  function initSection(section) {
    ...
    function initDesktop() {
      ...
      // Header visibility and the section's own rounded top corners both only
      // make sense together: while truly pinned full-viewport there's nothing
      // above the section to hide behind or round away from — same reasoning
      // for the fixed-position sticky "Book a meeting" button, which would
      // otherwise keep floating on top of the pinned section too. One
      // function keeps all three from ever getting out of sync.
      function setFullscreenActive(active) {
        if (headerEl) headerEl.classList.toggle('hdr--hidden', active);
        if (stickyBtnEl) stickyBtnEl.classList.toggle('hh-sticky-btn--pinned-hidden', active);
        section.classList.toggle('ind-section--pinned', active);
      }
      ...
      var st = ScrollTrigger.create({
        ...
        // Scrolling down into the pin (section about to fill the viewport)
        // and scrolling back up into it from below: activate either way.
        onEnter:     function () { setFullscreenActive(true); },
        onEnterBack: function () { setFullscreenActive(true); },
        // Scrolling down past the end of the pin (all industries done) and
        // scrolling back up out of it entirely: deactivate.
        onLeave:     function () { setFullscreenActive(false); },
        onLeaveBack: function () { setFullscreenActive(false); },
      });
