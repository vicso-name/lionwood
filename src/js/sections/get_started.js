/**
 * Get Started Slider — custom card stack
...
(function () {
  ...
  function initGetStarted(section) {
    var wrapper   = section.querySelector('.gs-cards-wrapper');
    var indicator = section.querySelector('[data-gs-indicator]');
    var prevBtn   = section.querySelector('.gs-nav__btn--prev');
    var nextBtn   = section.querySelector('.gs-nav__btn--next');

    if (!wrapper) return;

    var cards = Array.prototype.slice.call(wrapper.querySelectorAll('.gs-slide'));
  ...
  function init() {
    document.querySelectorAll('.gs-section').forEach(initGetStarted);
  }
