...
(function () {
  ...
  function init() {
    ...
    // Extra height was (total - 1) * STEP — enough for total-1 *transitions*
    // between products, but that leaves the last product with zero scroll
    // budget of its own: the instant `scrolled` reaches its threshold, the
    // wrapper's bottom edge arrives at the same moment and the sticky pin
    // releases immediately, so the last item never gets to be seen before
    // scrolling carries straight through into the next section. Every
    // product (0..total-1) needs its own full STEP, so the total extra
    // height must be total * STEP, not (total - 1) * STEP.
    function setWrapperHeight() {
      if (isMobile) {
        wrapper.style.height = '';
        return;
      }
      wrapper.style.height = (window.innerHeight + total * STEP) + 'px';
    }
